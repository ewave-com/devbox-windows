...
module DockerSync
  module SyncStrategy
    class Unison
      ...
      def sync(force = nil)
        # Ewave change, add option 'watch_only' to avoid forced sync before watching, and force flag
        if @options['watch_only'] && !force
          say_status 'ok', 'Sync skipped, only watch is available by enabled flag `watch_only`', :white if @options['verbose']
          return 0
        end
        # Ewave change end
